...
class DataLoaderParser:
    ...
    Handles:
    - Element_Vector.json: Contains element data with atom densities and mole percentages
    ...
    def load_all_data(self) -> Tuple[Dict, Dict, Dict]:
        ...
        # Load fuel salt data
        fuel_salt_path = os.path.join(self.base_directory, "Element_Vector.json")
    ...
    def load_fuel_salt_data(self, file_path: str) -> Dict:
        """
        Load and parse the Element_Vector.json file.
    ...
    def save_processed_data(self, output_directory: str) -> None:
        ...
        # Print statistics about loaded data
        if self.fuel_salt_data:
            print(f"Fuel salt data contains {len(self.fuel_salt_data.get('surrogate_vector', {}).get('0', {}))} elements")

        if self.surrogate_vector:
            print(f"Surrogate vector contains {len(self.surrogate_vector.get('surrogate_vector', {}).get('0', {}))} surrogates")

        if self.thermochimica_data:
            print(f"Thermochimica data contains {len(self.thermochimica_data)} timesteps")
